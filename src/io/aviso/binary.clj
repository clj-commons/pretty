...
(defn- format-line
  [offset ^bytes data line-count]
  (format
    "%04X:%s%n"
    offset
    (apply str
           (for [i (range line-count)]
             (format " %02X" (aget data (+ offset i)))))))

(defn format-byte-array
  "Formats an array of bytes into a hex-dump string, consisting of multiple lines; each line formatted as:
  ...
  (loop [offset 0
         blocks []]
    (let [remaining (- (alength data) offset)]
      (if (< remaining 1)
        (apply str blocks)
...
(defn- match?
  ...
  (and
    ...
    (== (aget data byte-offset) (aget alternate byte-offset))))

(defn- to-hex
  [byte-array byte-offset]
  (format "%02X" (aget byte-array byte-offset)))
...
(defn format-byte-array-delta
  ...
  [expected actual]
  (let [expected-length (alength expected)
        actual-length (alength actual)
